...
struct Projectile {
    ...
    new (rng: time.Rng, ctx: win.Window, width: int, height: int) {
        self.x = rng.range(-100, width + 100)
        self.y = self.x
        if rng.coin() {
            self.x *= -1
        }
...
fun main() {
    ...
    let ctx = win.WinBuilder().width(width).height(height).build()
    let event: win.Event?
    ...
    let i = 0
    while i < 5000 {
        projectiles.push(Projectile(rng, ctx, width, height))
        i += 1
    }
    loop "main": {
        keys.frame()
        loop "events": {
            ...
            if event.code() as int == win.Events.KeyPressed && event.key() as int == win.Keys.Escape {
                break "main"
            }
